...
1. **Kahn's Algorithm (BFS 기반 위상 정렬)**:
   - 각 정점의 `in_degree` (진입 차수)를 계산하고, `in_degree == 0`인 노드들을 큐에 넣습니다.
   - 큐에서 노드를 꺼낼 때마다 연결된 간선을 제거(`in_degree[next]--`)하고, 새로 0이 된 노드를 큐에 추가합니다.
2. **사이클 판별**:
   - 큐를 거쳐 나온 노드의 개수가 전체 노드 수 $N$보다 작다면 그래프 내에 **사이클(순환 참조)**이 존재하여 위상 정렬이 불가능함을 의미합니다.
3. **우선순위 조건 ("가능하면 작은 번호부터")**:
   - 일반 `queue` 대신 `priority_queue<int, vector<int>, greater<int>>`를 사용하면 번호가 작은 문제/작업부터 순서를 매길 수 있습니다 (BOJ 1766 문제집).
...
| :--- | :--- | :---: | :--- |
| **SWEA** | [1267. [S/W 문제해결 응용] 10일차 - 작업순서](https://swexpertacademy.com/main/code/problem/problemDetail.do?contestProbId=AV15FZuqAL4CFAYD) | D6 (체감 D4) | SWEA 대표 위상 정렬 문제. 선행 작업 조건이 주어질 때 전체 작업 순서 도출 |
| **백준** | [2252. 줄 세우기](https://www.acmicpc.net/problem/2252) | 골드 3 | 두 학생의 키 비교 결과가 주어질 때 줄을 세우는 위상 정렬 표준 문제 |
| **백준** | [1005. ACM Craft](https://www.acmicpc.net/problem/1005) | 골드 3 | 위상 정렬 + DP. 건물 건설 선행 관계와 소요 시간을 종합하여 특정 건물 완성 최소 시간 계산 |
| **백준** | [1766. 문제집](https://www.acmicpc.net/problem/1766) | 골드 2 | 위상 정렬 + Min-Heap. 선행 조건을 만족하면서 번호가 작은 문제부터 풀기 |
