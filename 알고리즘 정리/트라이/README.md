...
1. **정적 배열 크기 설정 공식**:
   - `MAX_NODES = (단어의 최대 개수 * 단어의 최대 길이) + 1`
   - 예: 단어 10,000개, 길이 10글자 $\to$ 노드는 최대 100,000개. `trie[100005][26]`는 약 10MB로 매우 안전합니다.
   - 무작정 500만 크기(`trie[5000000][26]`)를 잡으면 500MB가 넘어 메모리 초과(MLE)를 받습니다!
2. **단어 끝 플래그 (`is_end`)**: 접두사 검색(`startsWith`)인지 완전 단어 일치(`search`)인지 구분하기 위해 필수입니다.
3. **카카오 단골 주제**: 접두사 검색, 자동 완성, 사전순 정렬 등 문자열 Trie 문제는 카카오 블라인드 테스트의 단골 킬러 문항입니다.
...
| :--- | :--- | :---: | :--- |
| **Programmers** | [가사 검색](https://school.programmers.co.kr/learn/courses/30/lessons/60060) | Lv.4 | 2020 카카오 블라인드 기출. 단어 길이별 Trie 구축 + 접미사 와일드카드 처리용 역방향 Trie 활용 |
| **Programmers** | [자동완성](https://school.programmers.co.kr/learn/courses/30/lessons/17685) | Lv.4 | 2018 카카오 블라인드 기출. 각 Trie 노드에 해당 노드를 지나는 단어 개수를 카운트하여 최소 타이핑 수 계산 |
| **백준** | [14425. 문자열 집합](https://www.acmicpc.net/problem/14425) | 실버 3 | 문자열 집합에 특정 문자열이 존재하는지 검사하는 Trie/해시 기본 문제 |
| **백준** | [5052. 전화번호 목록](https://www.acmicpc.net/problem/5052) | 골드 4 | 한 번호가 다른 번호의 접두어가 되는지 일관성을 판별하는 Trie 대표 문제 |
