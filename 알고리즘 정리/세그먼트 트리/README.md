...
1. **배열 크기 설정**:
   - 원소 수 $N$에 대해 세그먼트 트리 배열의 크기는 $4N$으로 잡으면 메모리가 안전합니다 (`int tree[4 * MAX];`).
2. **점 갱신 vs 구간 갱신**:
   - 단일 원소 값만 변경된다면 **기본 세그먼트 트리** ($O(\log N)$).
   - 특정 구간 $[L, R]$ 전체에 값을 더하거나 변경해야 한다면 **Lazy Propagation** ($O(\log N)$) 필수.
3. **인덱스 주의**: 세그먼트 트리는 루트가 1번 노드, 왼쪽 자식 $2 \times idx$, 오른쪽 자식 $2 \times idx + 1$로 구현할 때 비트 연산(`idx << 1`, `idx << 1 | 1`)으로 빠르게 접근할 수 있습니다.
...
| :--- | :--- | :---: | :--- |
| **백준** | [2042. 구간 합 구하기](https://www.acmicpc.net/problem/2042) | 골드 1 | 점 갱신과 구간 합 쿼리를 수행하는 세그먼트 트리 표준 문제 |
| **백준** | [10868. 최솟값](https://www.acmicpc.net/problem/10868) | 골드 1 | 특정 구간의 최솟값을 $O(\log N)$에 반환하는 세그먼트 트리 |
| **백준** | [10999. 구간 합 구하기 2](https://www.acmicpc.net/problem/10999) | 플래티넘 4 | 구간 갱신과 구간 합을 처리하는 Lazy Propagation 세그먼트 트리 표준 |
| **Programmers** | [인사고과](https://school.programmers.co.kr/learn/courses/30/lessons/152995) | Lv.3 | 정렬 후 점수 트래킹(세그먼트 트리 또는 최대값 유지)을 통한 석차 산출 |
