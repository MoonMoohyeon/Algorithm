...
`[]` 연산자는 `insert`를 완벽하게 대체하지 않아요. 둘은 비슷해 보이지만 **역할과 동작 방식에 중요한 차이**가 있어서 상황에 맞게 구별해서 써야 해요.

결론부터 말하면, **`[]` 연산자는 편하게 값을 넣거나 수정할 때** 쓰고, **`insert`는 삽입 성공 여부를 확인하거나 기존 값을 덮어쓰지 않아야 할 때** 사용해요.
...
`[]` 연산자만 쓰면 안 되는 대표적인 두 가지 상황이 있어요.

#### **1. 기존 값을 덮어쓰면 안 될 때**

`[]` 연산자는 Key가 이미 존재하면 값을 **무조건 덮어씁니다**. 만약 Key가 없을 때만 값을 넣고, 있을 때는 기존 값을 유지하고 싶다면 `insert`를 써야 해요.

`insert`는 삽입에 성공했는지(`true`), 아니면 이미 Key가 존재해서 실패했는지(`false`)를 알려주거든요.
...
`[]` 연산자는 **Key가 없으면 자동으로 `(Key, 기본값)` 쌍을 만들어버리는** 아주 중요한 특징이 있어요. 이는 의도치 않은 원소 생성을 유발할 수 있죠.

예를 들어, 맵에 특정 Key가 있는지 확인하려고 아래처럼 코드를 짜면, `"melon"`이 없었더라도 `{ "melon", 0 }`이라는 원소가 새로 생겨버려요.
...
### **결론**
  ...
  * **속도가 최우선**이고 **정렬이 필요 없다면**? → \*\*`unordered_map`\*\*을 쓰세요. (대부분의 경우)
  * **Key를 기준으로 정렬**해야 하거나, **범위 기반 검색**이 필요하다면? → \*\*`map`\*\*을 쓰세요.