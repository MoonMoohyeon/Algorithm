...
1. **대각선 검사 최적화 ($O(1)$ 판별)**:
   - 2차원 배열 대신 1차원 방문 배열 3개로 $O(1)$ 검사:
     - 열 점유: `col_used[c]`
     - 대각선 1 (/ 방향): `diag1[r + c]`
     - 대각선 2 (\ 방향): `diag2[r - c + N]`
   - 이렇게 하면 매번 이전 퀸들의 위치를 반복문으로 검사하지 않아도 되어 실행 속도가 5~10배 빨라집니다.
2. **체스판 대칭성 활용**: 첫 번째 행의 절반($1 \dots N/2$)만 탐색하고 결과에 $\times 2$를 해주면 대칭성에 의해 탐색 시간을 절반으로 단축할 수 있습니다.
...
| :--- | :--- | :---: | :--- |
| **SWEA** | [2806. N-Queen](https://swexpertacademy.com/main/code/problem/problemDetail.do?contestProbId=AV7GKs06AU0DFAXB) | D3 | SWEA N-Queen 표준 문제 ($N \le 10$) |
| **Programmers** | [N-Queen](https://school.programmers.co.kr/learn/courses/30/lessons/12952) | Lv.3 | 프로그래머스 N-Queen 문제 ($N \le 12$) |
| **백준** | [9663. N-Queen](https://www.acmicpc.net/problem/9663) | 골드 4 | $N=14$까지 주어지므로 대각선 배열을 이용한 $O(1)$ 유망성 검사가 필수적인 문제 |
