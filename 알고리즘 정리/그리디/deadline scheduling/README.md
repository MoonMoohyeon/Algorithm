...
1. **우선순위 큐(Min-Heap)를 활용한 최적화**:
   - 마감일 순으로 정렬한 뒤, 현재 작업을 일단 min-heap에 넣습니다.
   - 만약 `heap.size() > current_deadline`이라면, 큐에서 가장 이익이 적은 작업(`heap.top()`)을 제거합니다.
   - 최종적으로 힙에 남은 작업들의 합이 최대 이익이 됩니다 ($O(N \log N)$ 풀이).
...
| :--- | :--- | :---: | :--- |
| **백준** | [1781. 컵라면](https://www.acmicpc.net/problem/1781) | 골드 2 | 데드라인 스케줄링의 대표 문제 (우선순위 큐를 이용한 $O(N \log N)$ 최적화) |
| **백준** | [13904. 과제](https://www.acmicpc.net/problem/13904) | 골드 3 | 마감일과 점수가 주어질 때 점수 합 최대화 |
| **백준** | [1715. 카드 정렬하기](https://www.acmicpc.net/problem/1715) | 골드 4 | 항상 가장 작은 두 카드 묶음을 우선순위 큐로 합치는 그리디 |
