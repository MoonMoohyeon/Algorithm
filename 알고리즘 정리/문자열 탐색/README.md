...
1. **$\pi$ 배열 (LPS 배열)의 의미**:
   - `pi[i]`: 길이 `i+1` 접두사에서 접두사이면서 접미사인 가장 긴 부분 문자열의 길이.
2. **KMP 시간 복잡도**: 텍스트 길이 $N$, 패턴 길이 $M$일 때 $O(N + M)$ 보장.
3. **코딩테스트 출제 경향**: 단순 문자열 검색은 `std::string::find`나 슬라이딩 윈도우, 해시로 해결 가능한 경우가 많지만, 패턴이 매우 길거나 반복 주기를 찾는 문제(예: `L - pi[L-1]`)에서는 KMP가 유일한 해법입니다.
...
| :--- | :--- | :---: | :--- |
| **Programmers** | [문자열 압축](https://school.programmers.co.kr/learn/courses/30/lessons/60057) | Lv.2 | 2020 카카오 블라인드 기출. 1개부터 $N/2$개 단위까지 문자열을 잘라 반복되는 패턴 탐색 |
| **백준** | [1786. 찾기](https://www.acmicpc.net/problem/1786) | 골드 5 | KMP 알고리즘의 교과서 문제 (패턴이 나타난 총 횟수와 모든 시작 위치 출력) |
| **백준** | [1305. 광고](https://www.acmicpc.net/problem/1305) | 플래티넘 4 | $\pi$ 배열의 끝값 `pi[L-1]`을 이용하여 반복되는 최소 광고 길이($L - pi[L-1]$) 계산 |
